 ...
 * Default caching and chunking parameters.                                    *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
void show_help(char * msg) {
    printf("\n%s\n", msg);
    printf("Usage:  simon  [-v]  OutputHDF5File\n\n-v : verbose logging\n\n");
    exit(1);
}
...
int main(int argc, char **argv) {
     ...
     */
    printf("simon: Data and header initialisation completed.  Begin data writes .....\n");
    time(&time1);
    if(wrh5_open(&wrh5_ctx, &wrh5_hdr, NULL, NULL, path_h5, verbose) != 0) {
